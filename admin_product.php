<?php 
	...
	if (isset($_POST['add_product'])) {
		...
		if(mysqli_num_rows($select_product_name) > 0){
			...
			$insert_product = mysqli_query($conn, "INSERT INTO `products`(`name`, `price`, `product_detail`, `image`,`category`) VALUES('$product_name', '$product_price', '$product_detail', '$image','$product_category')") or die('query failed');
			if($insert_product){
	...
	if (isset($_POST['update_product'])) {
		...
		$update_query = mysqli_query($conn, "UPDATE `products` SET id='$update_p_id', name='$update_p_name',price='$update_p_price',product_detail='$update_p_detail',category='$update_p_category',image='$update_p_img' WHERE id='$update_p_id'") or die('query failed');
	...
	<section class="update-container">
		...
		<form method="post" action="" enctype="multipart/form-data">
			...
			<textarea name="update_p_detail"><?php echo $fetch_edit['product_detail']; ?></textarea>
			  <select name="update_p_category" required>
                <option value="Formals" <?php if($fetch_edit['category']=="Formals") echo "selected"; ?>>Formals</option>
                <option value="Traditionals" <?php if($fetch_edit['category']=="Traditionals") echo "selected"; ?>>Traditionals</option>
                <option value="Jeans" <?php if($fetch_edit['category']=="Jeans") echo "selected"; ?>>Jeans</option>
                <option value="Casual Shirts" <?php if($fetch_edit['category']=="Casual Shirts") echo "selected"; ?>>Casual Shirts</option>
                <option value="Winter Wear" <?php if($fetch_edit['category']=="Winter Wear") echo "selected"; ?>>Winter Wear</option>
            </select>
